...
class Environment:
    ...
    def round(self, _user_class, pulled_arm, optimal_bid):
        ...
        positives = np.random.binomial(1, self.get_conversion_prob(self.prices[pulled_arm], _user_class),
                                       np.round(tests).astype(int))
        reward = np.sum(positives) * (self.prices[pulled_arm] - self.prod_cost) - self.get_costs(optimal_bid,
                                                                                                 _user_class)
    ...
    def part3_round(self, _user_class, pulled_arm_price, pulled_arm_bid):
        ...
        reward = np.sum(positives) * (self.prices[pulled_arm_price] - self.prod_cost) - max(0, self.sample_costs(
            pulled_arm_bid, _user_class))
        return reward, tests, np.sum(positives)
    ...
    def seasonal_prob_round(self, _user_class, phase, pulled_arm, optimal_bid):
        ...
        reward = tests * tested_conv_prob * (self.prices[pulled_arm] - self.prod_cost) - self.get_costs(optimal_bid,
                                                                                                        _user_class)
        return reward, tests
